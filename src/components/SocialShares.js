...
const ShareButtons = ({ report }) => {
  ...
  // Memoized share URLs for the current report
  const shareUrl = useMemo(() => {
    const shareTitle = `Missing Person: ${report.fullName}`;
  ...
  return (
    <div className="flex flex space-x-2 pb-4 pr-2 w-full justify-end">
      ...
      <TwitterShareButton url={shareUrl} title={`Missing Person: ${report.fullName}`}>
      ...
      <EmailShareButton
        // url={shareUrl}
        url="https://www.hopehelpers.netlify.app"
        subject={`Missing Person: ${report.fullName}`}
        body={`Check out this missing person report: ${shareUrl}`}
      >
